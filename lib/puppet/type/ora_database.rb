...
module Puppet
  #
  # Create a new type ora_database
  #
  class Type
    newtype(:ora_database) do
      ...
      set_command(%i[sql remove_directories srvctl orapwd])
      ...
      def create_ora_pwd_file(command_builder)
        command_builder.add(
          "file=#{oracle_home}/dbs/orapw#{@dbname} force=y password=#{sys_password} entries=20", :orapwd, :sid => @dbname
        )
      end
